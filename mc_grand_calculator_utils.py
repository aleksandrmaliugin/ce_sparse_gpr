...
def adsorption_coverage_denominator(sites: Sequence[AdsorptionSite]) -> int:
    """Return the denominator used for CO coverage.

    Coverage is reported relative to the number of surface atoms, which is
    equal to the number of ontop adsorption sites in the generated site grid.
    Bridge sites are still valid adsorption sites, but they do not increase
    the coverage denominator.
    """
    sites = validate_sites(sites)
    n_ontop = int(sum(site.kind == "ontop" for site in sites))
    if n_ontop <= 0:
        raise ValueError(
            "Cannot define CO coverage: no ontop sites were found. "
            "The coverage denominator is the number of surface atoms / ontop sites."
        )
    return n_ontop


def co_coverage(occupation: np.ndarray, coverage_denominator: int) -> float:
    """Return theta_CO = N_CO / N_surface_atoms.

    Here N_surface_atoms is represented by the number of ontop sites, not by
    the total number of ontop + bridge adsorption sites.
    """
    occupation = np.asarray(occupation, dtype=bool)
    coverage_denominator = int(coverage_denominator)
    if coverage_denominator <= 0:
        raise ValueError(f"coverage_denominator must be positive, got {coverage_denominator}.")
    return float(int(occupation.sum()) / coverage_denominator)
...
def attach_mc_info(
    ...
    if coverage_denominator is None:
        # Backward-compatible fallback for external calls.  The MC driver passes
        # the physically intended denominator explicitly: number of ontop sites.
        coverage_denominator = n_sites
    coverage_denominator = int(coverage_denominator)
    if coverage_denominator <= 0:
        raise ValueError(f"coverage_denominator must be positive, got {coverage_denominator}.")
    ...
    atoms.info["N_ontop_sites"] = coverage_denominator
    atoms.info["theta_CO"] = co_coverage(occupation, coverage_denominator)
